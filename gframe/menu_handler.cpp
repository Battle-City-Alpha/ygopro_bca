	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			...
			case BUTTON_JOIN_HOST: {
				...
				if(remote_addr == -1) {
					...
					if(status != 0) {
						...
						mainGame->stACMessage->setText(dataManager.GetSysString(1412));
			...
			case BUTTON_HOST_CONFIRM: {
				...
				if (mainGame->KaibotHostMode)
					mainGame->LaunchBot(mainGame->cbBotDecks->getItem(mainGame->cbBotDecks->getSelected()));
			...
			case BUTTON_REPLAY_SAVE: {
				...
				if(prev_operation == BUTTON_RENAME_REPLAY) {
					...
					if(Replay::RenameReplay(mainGame->lstReplayList->getListItem(prev_sel), newname)) {
						...
						mainGame->stACMessage->setText(dataManager.GetSysString(1365));
						mainGame->PopupElement(mainGame->wACMessage, 100);
					}
				}
				prev_operation = 0;
