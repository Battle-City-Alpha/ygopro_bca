...
restart:
	...
	int start_hand = 5;
	if (mainGame->handtest_mode)
		start_hand = mainGame->gameConf.number_card_hand_test_hand;
	...
	size_t slen = 0;
	if (mainGame->handtest_mode) {
		opt = DUEL_ATTACK_FIRST_TURN | 5 << 16 | DUEL_SIMPLE_AI;
		...
		rh.flag = REPLAY_SINGLE_MODE | REPLAY_TEST_HAND;
		rh.seed = seed;
		...
		if (mainGame->gameConf.shuffle_deck_test_hand)
		{
			std::random_device rd;
			std::mt19937 g(rd());
			std::shuffle(playerdeck.main.begin(), playerdeck.main.end(), g);
		}

		mainGame->dField.Initial(0, playerdeck.main.size(), playerdeck.extra.size());
		last_replay.WriteInt32(playerdeck.main.size(), false);
		for (int32 i = (int32)playerdeck.main.size() - 1; i >= 0; --i)
		{
			new_card(pduel, playerdeck.main[i]->first, 0, 0, LOCATION_DECK, 0, POS_FACEDOWN_DEFENSE);
			mainGame->dField.deck[0][i]->code = playerdeck.main[i]->first;
			last_replay.WriteInt32(playerdeck.main[i]->first, false);
		}
		last_replay.WriteInt32(playerdeck.extra.size(), false);
		for (int32 i = 0; i < (int32)playerdeck.extra.size(); ++i) {
			new_card(pduel, playerdeck.extra[i]->first, 0, 0, LOCATION_EXTRA, 0, POS_FACEDOWN_DEFENSE);
			mainGame->dField.extra[0][i]->code = playerdeck.extra[i]->first;
			last_replay.WriteInt32(playerdeck.extra[i]->first, false);
		}
		...
		preload_script(pduel, cmd, sizeof(cmd) - 1);
