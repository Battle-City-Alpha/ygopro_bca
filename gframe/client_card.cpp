...
bool ClientCard::deck_sort_quantity(code_pointer p1, code_pointer p2) {
	int qty1 = mainGame->chest.GetCardAmount((p1->second.alias == 0) ? (p1->first) : (p1->second.alias));
	int qty2 = mainGame->chest.GetCardAmount((p2->second.alias == 0) ? (p2->first) : (p2->second.alias));
	return qty1 > qty2;
}
}
