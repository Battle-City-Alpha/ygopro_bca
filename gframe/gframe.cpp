...
int main(int argc, char* argv[]) {
	...
	for(int i = 1; i < wargc; ++i) {
		...
		else if (!wcscmp(wargv[i], L"-b")) { // Create host for bot duel
			exit_on_return = !keep_on_return;
			ygo::mainGame->HideElement(ygo::mainGame->wMainMenu);
			ygo::mainGame->KaibotHostMode = true;
			ygo::mainGame->gameConf.forced = false;
			ygo::mainGame->UpdatewCreateHostWithBot();
			ClickButton(ygo::mainGame->btnCreateHost);
			break;
		}
		else if(!wcscmp(wargv[i], L"-j")) { // Join host
			exit_on_return = !keep_on_return;
			if (i + 2 < wargc) {
				wcscpy(ygo::mainGame->gameConf.lasthost, wargv[++i]);
				wcscpy(ygo::mainGame->gameConf.lastport, wargv[++i]);
				ygo::mainGame->ebJoinHost->setText(ygo::mainGame->gameConf.lasthost);
				ygo::mainGame->ebJoinPort->setText(ygo::mainGame->gameConf.lastport);
			}
			ygo::mainGame->HideElement(ygo::mainGame->wMainMenu);
