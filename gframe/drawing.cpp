...
void Game::DrawMisc() {
	...
	DrawShadowText(numFont, dInfo.strLP[1], Resize(691, 12, 992, 30), Resize(0, 1, 1, 0), mainGame->playerlpcolor, 0xff000000, true, false, 0);

	recti p1size = Resize(447, 44, 600, 66);
    recti p2size = Resize(872, 44, 872, 66);
...
void Game::DrawSpec() {
	...
	for(int i = 0; i < 8; ++i) {
		...
		if(chatTiming[i]) {
			...
			recti rectloc(mainGame->wChat->getRelativePosition().UpperLeftCorner.X + mainGame->cbChatSelect->getAbsolutePosition().getWidth(), mainGame->window_size.Height - 45, mainGame->wChat->getRelativePosition().UpperLeftCorner.X + 2 + w + mainGame->cbChatSelect->getAbsolutePosition().getWidth(), mainGame->window_size.Height - 25);
			rectloc -= position2di(0, i * 20);
			recti msgloc(mainGame->wChat->getRelativePosition().UpperLeftCorner.X + mainGame->cbChatSelect->getAbsolutePosition().getWidth(), mainGame->window_size.Height - 45, mainGame->wChat->getRelativePosition().UpperLeftCorner.X - 4, mainGame->window_size.Height - 25);
...
void Game::DrawThumb(code_pointer cp, position2di pos, const std::unordered_map<int,int>* lflist, bool drag) {
	...
	if (lcode == 0)
		lcode = code;
	irr::video::ITexture* img = imageManager.GetTexture(code, true);
	if (img == NULL)
...
static void DrawPlayerAvatar(IVideoDriver* driver, vector2di pos, int player, bool right)
{
	...
	if (avatar)
	{
		...
		position2di maxSize = mainGame->Resize(AVATAR_SIZE, AVATAR_SIZE);
		...
		if ((mainGame->dInfo.turn % 2 && mainGame->dInfo.isFirst) || (!(mainGame->dInfo.turn % 2) && !mainGame->dInfo.isFirst)) {
			if (right) {
				//driver->draw2DRectangleOutline(destRect, 0xffffffff);
				driver->draw2DRectangle(irr::video::SColor::SColor(120, 255, 255, 255), destRect);
			}
			else {
				//driver->draw2DRectangleOutline(destRect, 0xffff8080);
			}
		}
		else {
			if (right)
...
void Game::DrawAvatars()
{
	...
	position2di p1pos = mainGame->Resize(330, 27);
	position2di p2pos = mainGame->Resize(989, 27);
	p1pos.Y += 3;
	p2pos.Y += 3;
...
static void DrawPlayerBorder(IVideoDriver* driver, recti pos, int player, bool right)
{
	...
	if (border)
	{
		...
		if (right)
			sourceRect = recti(dimension2di(0, 0), dimension2di(-border->getSize().Width, border->getSize().Height));
...
void Game::DrawBorders()
{
	...
	recti p1pos = mainGame->Resize(327, 7, 633, 143);
	recti p2pos = mainGame->Resize(686, 7, 992, 143);

	if (!dInfo.isTag || !dInfo.tag_player[0])
		DrawPlayerBorder(driver, p1pos, trueIds[0], false);
	else
		DrawPlayerBorder(driver, p1pos, trueIds[1], false);
	if (!dInfo.isTag || !dInfo.tag_player[1])
		DrawPlayerBorder(driver, p2pos, trueIds[2], true);
	else
		DrawPlayerBorder(driver, p2pos, trueIds[3], true);
}
void Game::SwapCovers(bool force)
{
	int n = dInfo.isTag ? 4 : 2;
	for (int i = 0; i < n; i++)
		if (!imageManager.tCover[i])
			return;
	int trueIds[4];
	bool isHostTeam = DuelClient::IsHostTeam();
	if (dInfo.isTag)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			ITexture* temp = imageManager.tCover[i];
			imageManager.tCover[i] = imageManager.tCover[trueIds[i]];
			imageManager.tCover[trueIds[i]] = temp;
		}
	}
	else
	{
		...
		if (!isHostTeam || force)
		{
			ITexture* temp = imageManager.tCover[0];
			imageManager.tCover[0] = imageManager.tCover[1];
			imageManager.tCover[1] = temp;
		}
	}
	IsCoverSwap = true;
}
bool Game::CheckBCATexturesLoaded()
{
	int n = dInfo.isTag ? 4 : 2;
	for (int i = 0; i < n; i++)
