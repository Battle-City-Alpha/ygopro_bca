...
bool Game::Initialize() {
	...
	window_size = irr::core::dimension2d<u32>(1024, 640);
	...
	device->setWindowCaption(L"YGOPro - Battle City Alpha");
...
//	btnTestMode = env->addButton(rect<s32>(10, 135, 270, 165), wMainMenu, BUTTON_TEST_MODE, dataManager.GetSysString(1203));
	...
	cbBotDecks = env->addComboBox(rect<s32>(140, 55, 300, 80), wCreateHost);
	RefreshBotDeck(cbBotDecks);
	...
	stHostPrepRule = env->addStaticText(L"", rect<s32>(280, 30, 460, 270), false, true, wHostPrepare);
	...
	imgCard->setImage(imageManager.tUnknown);
	showingcode = 0;
	...
	//tab
	wInfos = env->addTabControl(rect<s32>(1, 275, 301, 640), 0, true);
	...
	stBoosterInfo = env->addStaticText(L"", rect<s32>(15, 85, 296, 108), false, true, tabInfo, -1, false);
	stBoosterInfo->setOverrideColor(SColor(255, 0, 0, 255));
	...
	//helper
	tabHelperOwner = wInfos->addTab(dataManager.GetSysString(1298));
	tabHelperOwner->setRelativePosition(recti(16, 49, 299, 362));
	tabHelper = env->addWindow(recti(0, 0, 250, 300), false, L"", tabHelperOwner);
	...
	scrTabHelper = env->addScrollBar(false, rect<s32>(252, 0, 272, 300), tabHelperOwner, SCROLL_TAB_HELPER);
	...
	posY += 27;
	stSkins = env->addStaticText(dataManager.GetSysString(2999), rect<s32>(posX, posY, posX + 50, posY + 25), false, false, tabHelper);
	cbSkins = env->addComboBox(rect<s32>(posX + 60, posY, posX + 160, posY + 25), tabHelper, COMBOBOX_SKINS);
	skinSystem = new CGUISkinSystem("skins", device);
	core::array<core::stringw> skins = skinSystem->listSkins(); 
	
	wchar_t skin_name[256];
	swprintf_s(skin_name, L"%d", -1);
	cbSkins->addItem(skin_name);
	for (int i = 0; i < skins.size(); i++)
	{		
		swprintf_s(skin_name, L"%d", i);
		cbSkins->addItem(skin_name);
	}
	posY += 27;
	chkShuffleHandTest = env->addCheckBox(false, rect<s32>(posX, posY, posX + 240, posY + 25), tabHelper, CHECKBOX_SHUFFLE_HAND_TEST, dataManager.GetSysString(2998));
	chkShuffleHandTest->setChecked(gameConf.shuffle_deck_test_hand != 0);
	posY += 27;
	stCardsCountHandTest = env->addStaticText(dataManager.GetSysString(2997), rect<s32>(posX, posY, posX + 155, posY + 20), false, false, tabHelper);
	cbCardsCountHandTest = env->addComboBox(rect<s32>(posX + 160, posY, posX + 240, posY + 25), tabHelper, COMBOBOX_START_HAND_COUNT_HAND_TEST);	
	for (int i = 0; i <= 5; i++)
	{
		wchar_t count[256];
		swprintf_s(count, L"%d", i);
		cbCardsCountHandTest->addItem(count);
	}
	cbCardsCountHandTest->setSelected(gameConf.number_card_hand_test_hand);
	posY += 27;
	chkShowBoosterTagDeckEditor = env->addCheckBox(false, rect<s32>(posX, posY, posX + 240, posY + 25), tabHelper, CHECKBOX_SHOW_BOOSTER_INFOS, dataManager.GetSysString(2996));
	chkShowBoosterTagDeckEditor->setChecked(gameConf.show_booster_infos != 0);


	//system
	tabSystemOwner = wInfos->addTab(dataManager.GetSysString(1273));
	tabSystemOwner->setRelativePosition(recti(16, 49, 299, 362));
	tabSystem = env->addWindow(recti(0, 0, 250, 300), false, L"", tabSystemOwner);
	...
	scrTabSystem = env->addScrollBar(false, rect<s32>(252, 0, 272, 300), tabSystemOwner, SCROLL_TAB_SYSTEM);
	...
	posY += 30;
	stDimension = env->addStaticText(dataManager.GetSysString(1282), rect<s32>(posX + 23, posY + 3, posX + 120, posY + 28), false, false, tabSystem);
	...
	//announce card
	wANCard = env->addWindow(rect<s32>(500, 170, 830, 370), false, L"");
	wANCard->getCloseButton()->setVisible(false);
	wANCard->setVisible(false);
	ebANCard = env->addEditBox(L"", rect<s32>(20, 30, 310, 50), true, wANCard, EDITBOX_ANCARD);
	ebANCard->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);
	lstANCard = env->addListBox(rect<s32>(20, 55, 310, 160), wANCard, LISTBOX_ANCARD, true);
	btnANCardOK = env->addButton(rect<s32>(100, 165, 230, 190), wANCard, BUTTON_ANCARD_OK, dataManager.GetSysString(1211));
	...
	btnHandTest = env->addButton(rect<s32>(205, 90, 295, 130), 0, BUTTON_HAND_TEST, dataManager.GetSysString(2048));
	...
	cbSortType->addItem(dataManager.GetSysString(2995));
	...
	stAttribute = env->addStaticText(dataManager.GetSysString(1319), rect<s32>(10, 22 + 50 / 6, 30, 42 + 50 / 6), false, false, wFilter);
	cbAttribute = env->addComboBox(rect<s32>(60, 20 + 50 / 6, 120, 40 + 50 / 6), wFilter, COMBOBOX_ATTRIBUTE);
	...
	cbRace = env->addComboBox(rect<s32>(125, 20 + 50 / 6, 200, 40 + 50 / 6), wFilter, COMBOBOX_RACE);
	cbRace->setMaxSelectionRows(10);
	cbRace->addItem(dataManager.GetSysString(1310), 0);
	for(int filter = 0x1; filter != 0x2000000; filter <<= 1)
		cbRace->addItem(dataManager.FormatRace(filter), filter);

	stBooster = env->addStaticText(dataManager.GetSysString(1321), rect<s32>(10, 40 + 75 / 6, 70, 60 + 75 / 6), false, false, wFilter);
	cbBoosters = env->addComboBox(rect<s32>(60, 40 + 75 / 6, 200, 60 + 75 / 6), wFilter, COMBOBOX_BOOSTERS);
	cbBoosters->addItem(dataManager.GetSysString(1310));
	for (std::pair<std::wstring, std::wstring> booster : boosters.BoosterTagLink)
		cbBoosters->addItem(booster.first.c_str());
	...
	chkSearchBCACardsOnly = env->addCheckBox(true, rect<s32>(8, 100, 90, 120), wFilter, CHECKBOX_BCA_SEARCH, L"BCA");
	...
	stPlayReplay = env->addStaticText(dataManager.GetSysString(1349), rect<s32>(360, 30, 570, 50), false, true, wReplay);
	stReplayInfo = env->addStaticText(L"", rect<s32>(360, 60, 570, 350), false, true, wReplay);
	stStartReplayTurn = env->addStaticText(dataManager.GetSysString(1353), rect<s32>(360, 275, 570, 295), false, true, wReplay);
	...
	stMainMessage = env->addStaticText(dataManager.GetSysString(1352), rect<s32>(360, 10, 550, 30), false, true, tabSingle);
	...
	stReplay = env->addStaticText(dataManager.GetSysString(1342), rect<s32>(20, 25, 290, 45), false, false, wReplaySave);
	...
	//restart single
	btnRestartSingle = env->addButton(rect<s32>(205, 215, 295, 245), 0, BUTTON_RESTART_SINGLE, dataManager.GetSysString(2049));
	btnRestartSingle->setVisible(false);
	btnReloadBCATextures = env->addButton(rect<s32>(205, 180, 295, 210), 0, BUTTON_RELOAD_TEXTURES, dataManager.GetSysString(2993));
	btnReloadBCATextures->setVisible(false);
	btnSwapCovers = env->addButton(rect<s32>(205, 215, 295, 245), 0, BUTTON_SWAP_COVERS, dataManager.GetSysString(2992));
	...
	cbSkins->setSelected(gameConf.skin_index + 1);
	LoadSkin();
	ApplySkin();
...
void Game::MainLoop() {
	...
	while(device->run()) {
		...
		gMutex.lock();
		if(dInfo.isStarted) {
			if (!CheckBCATexturesLoaded()) {
				imageManager.LoadBCACustomTextures();				
			}
			else {
				if (!IsCoverSwap) {
					SwapCovers();
		...
		if(cur_time >= 1000) {
			myswprintf(cap, L"YGOPro - Battle City Alpha - FPS: %d", fps);
	...
	//imageManager.DeleteBCACustomTextures();
...
void Game::LoadConfig() {
	...
	gameConf.shuffle_deck_test_hand = true;
	gameConf.number_card_hand_test_hand = 5;
...
#endif
		} else if (!strcmp(strbuf, "number_card_hand_test_hand")) {
			gameConf.number_card_hand_test_hand = atof(valbuf);
		}
		else if (!strcmp(strbuf, "shuffle_deck_test_hand")) {
			gameConf.shuffle_deck_test_hand = atof(valbuf);
		}
		else if (!strcmp(strbuf, "forced")) {
			gameConf.forced = atof(valbuf);
		}
		else if (!strcmp(strbuf, "autoclosereplay")) {
		gameConf.AutoCloseReplay = atof(valbuf);
		}
...
void Game::SaveConfig() {
	...
	fprintf(fp, "number_card_hand_test_hand = %d\n", gameConf.number_card_hand_test_hand);
	fprintf(fp, "forced = %d\n", gameConf.forced);
...
void Game::ShowCardInfo(int code, bool resize) {
	...
	if(cd.type & TYPE_MONSTER) {
		...
		if (!mainGame->chest.IsUnlimited()) {
			int alias = (cd.alias == 0) ? (code) : (cd.alias);
		...
		stInfo->setRelativePosition(recti(15, 37, 290 * window_size.Width / 1024, 60));
		int posY = 62;
		stDataInfo->setRelativePosition(recti(15, posY, 290 * window_size.Width / 1024, posY + 23));
		posY += 25;
		if (!dInfo.isStarted && mainGame->gameConf.show_booster_infos) {
			stBoosterInfo->setText(boosters.GetBoosters((cd.alias == 0) ? (code) : (cd.alias)).c_str());
			stSetName->setRelativePosition(rect<s32>(15, posY, 287, posY + 23));
			stBoosterInfo->setRelativePosition(recti(15, posY, 290 * window_size.Width / 1024, posY + 23));
			posY += 25;
		}
		stText->setRelativePosition(ResizeSizeOnly(15, posY, 280, 324));
		scrCardText->setRelativePosition(Resize(267, posY, 287, 324));
	} else {
		...
		if (!dInfo.isStarted && mainGame->gameConf.show_booster_infos) {
			stBoosterInfo->setText(boosters.GetBoosters((cd.alias == 0) ? (code) : (cd.alias)).c_str());
			stSetName->setRelativePosition(rect<s32>(15, 60, 290, 60 + offset));
			stBoosterInfo->setRelativePosition(recti(15, posY, 290 * window_size.Width / 1024, posY + 23));
			posY += 25;
		}
		stText->setRelativePosition(ResizeSizeOnly(15, posY, 280, 324));
...
void Game::AddChatMsg(const wchar_t* msg, int player, bool recieve) {
	for(int i = 7; i > 0; --i) {
	...
	chatMsg[0].clear();
	if (!recieve) {
		if (wcsncmp(msg, L"/t ", 3) == 0)
		{
			player = 12;
			msg += 3;
			std::wstring newmsg = msg;
			newmsg = L"[Tag][Me]: " + newmsg;

			chatTiming[0] = 1200;
			chatType[0] = player;
			chatMsg[0].append(newmsg);
			return;
		}
		else if (mainGame->dInfo.player_type == 7)
		{
			std::wstring newmsg = msg;
			if (!wcsncmp(msg, L"/o ", 3)) {
				player = 14;
				msg += 3;
				newmsg = L"[Observers chat][Me]: " + newmsg.substr(3);
			}
			else {
				player = 16;
				newmsg = L"[Me]: " + newmsg;
	...
	case 9: //error message
		...
		break;
	case 12:
		chatMsg[0].append(L"[Tag]");
		break;
	case 15:
		chatMsg[0].append(L"[Server]: ");
		break;
	case 16: // Observers normal message
		break;
	case 14: // Observers chat message
		chatMsg[0].append(L"[Observers Chat]");
		break;
...
void Game::OnResize() {
	...
	float xScale = (float)window_size.Width / 1024.0;
	float yScale = (float)window_size.Height / 640.0;
	...
	wANCard->setRelativePosition(ResizeWin(500, 170, 830, 370));
	...
	wCardImg->setRelativePosition(ResizeCardImgWin(1, 1, 199, 273));
	imgCard->setRelativePosition(ResizeCardImgWin(10, 9, 187, 263));
	wInfos->setRelativePosition(Resize(1, 273, 301, 640));
	...
	stBoosterInfo->setRelativePosition(recti(15, 85, 296 * window_size.Width / 1024, 108));
	...
	cbRace->setRelativePosition(Resize(150, 20 + 50 / 6, 225, 40 + 50 / 6));
	cbAttribute->setRelativePosition(Resize(80, 20 + 50 / 6, 145, 40 + 50 / 6));
	cbLimit->setRelativePosition(Resize(290, 3, 405, 23));
	ebStar->setRelativePosition(Resize(80, 75, 125, 92));
	ebScale->setRelativePosition(Resize(180, 75, 225, 92));
	...
	stCategory->setRelativePosition(Resize(5, 25 / 6 + 2, 80, 22 + 25 / 6));
	stLimit->setRelativePosition(Resize(235, 2 + 25 / 6, 295, 22 + 25 / 6));
	stAttribute->setRelativePosition(Resize(5, 22 + 50 / 6, 80, 42 + 50 / 6));
	stBooster->setRelativePosition(Resize(5, 40 + 75 / 6, 80, 60 + 75 / 6));
	cbBoosters->setRelativePosition(Resize(80, 40 + 75 / 6, 225, 60 + 75 / 6));
	stAttack->setRelativePosition(Resize(235, 22 + 50 / 6, 295, 42 + 50 / 6));
	stDefense->setRelativePosition(Resize(235, 42 + 75 / 6, 295, 62 + 75 / 6));
	stStar->setRelativePosition(Resize(5, 62 + 100 / 6, 80, 82 + 100 / 6));
	stSearch->setRelativePosition(Resize(235, 60 + 100 / 6, 295, 80 + 100 / 6));
	stScale->setRelativePosition(Resize(130, 62 + 100 / 6, 180, 82 + 100 / 6));
	...
	btnHandTest->setRelativePosition(ResizeButtonNearCardInfos(50, 90));

	btnLeaveGame->setRelativePosition(ResizeButtonNearCardInfos(5, 35));
	btnChainIgnore->setRelativePosition(ResizeButtonNearCardInfos(40, 70));
	btnSpectatorSwap->setRelativePosition(ResizeButtonNearCardInfos(75, 105));
	btnChainWhenAvail->setRelativePosition(ResizeButtonNearCardInfos(75, 105));
	btnChainAlways->setRelativePosition(ResizeButtonNearCardInfos(110, 140));
	btnShuffle->setRelativePosition(ResizeButtonNearCardInfos(145, 175));
	btnReloadBCATextures->setRelativePosition(ResizeButtonNearCardInfos(180, 210));
	btnSwapCovers->setRelativePosition(ResizeButtonNearCardInfos(215, 245));
	btnRestartSingle->setRelativePosition(ResizeButtonNearCardInfos(250, 290));

	btnCancelOrFinish->setRelativePosition(ResizeButtonNearCardInfos(230, 265));
	...
	chkSearchBCACardsOnly->setRelativePosition(Resize(8, 100, 90, 120));
	
	tabHelperOwner->setRelativePosition(Resize(16, 49, 299, 400));
	tabHelper->setRelativePosition(Resize(0, 0, 250, 400)); 
	int posX = 0;
	int posY = 0;
	chkMAutoPos->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 27;
	chkSTAutoPos->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 27;
	...
	posY += 27;
	chkQuickAnimation->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 27;
	chkAutoSaveReplay->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 27;
	stSkins->setRelativePosition(Resize(posX, posY, posX + 50, posY + 25));
	cbSkins->setRelativePosition(Resize(posX + 60, posY, posX + 160, posY + 25));
	posY += 27;
	chkShuffleHandTest->setRelativePosition(Resize(posX, posY, posX + 240, posY + 25));
	posY += 27;
	stCardsCountHandTest->setRelativePosition(Resize(posX, posY, posX + 155, posY + 20));
	cbCardsCountHandTest->setRelativePosition(Resize(posX + 160, posY, posX + 240, posY + 25));
	posY += 27;
	chkShowBoosterTagDeckEditor->setRelativePosition(Resize(posX, posY, posX + 240, posY + 25));
	chkShowBoosterTagDeckEditor->setChecked(gameConf.show_booster_infos != 0);

	posX = 0;
	posY = 0;
	tabSystemOwner->setRelativePosition(Resize(16, 49, 299, 362));
	tabSystem->setRelativePosition(Resize(0, 0, 250, 300));
	scrTabSystem->setRelativePosition(Resize(252, 0, 272, 300));
	chkIgnore1->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 30;
	chkIgnore2->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 30;
	chkIgnoreDeckChanges->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 30;
	chkAutoSearch->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 30;
	chkMultiKeywords->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 30;
	chkPreferExpansionScript->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	posY += 30; 
	stDimension->setRelativePosition(Resize(posX + 23, posY + 3, posX + 120, posY + 28));
	btnWinResizeS->setRelativePosition(Resize(posX + 115, posY, posX + 145, posY + 25));
	btnWinResizeM->setRelativePosition(Resize(posX + 150, posY, posX + 180, posY + 25));
	btnWinResizeL->setRelativePosition(Resize(posX + 185, posY, posX + 215, posY + 25));
	btnWinResizeXL->setRelativePosition(Resize(posX + 220, posY, posX + 250, posY + 25));
	posY += 30;
	chkEnableSound->setRelativePosition(Resize(posX, posY, posX + 120, posY + 25));
	scrSoundVolume->setRelativePosition(Resize(posX + 116, posY + 4, posX + 250, posY + 21));
	posY += 30;
	chkEnableMusic->setRelativePosition(Resize(posX, posY, posX + 120, posY + 25));
	scrMusicVolume->setRelativePosition(Resize(posX + 116, posY + 4, posX + 250, posY + 21));
	posY += 30;
	chkMusicMode->setRelativePosition(Resize(posX, posY, posX + 260, posY + 25));
	
	if (displayedcard > 0)
		ShowCardInfo(displayedcard);
}
recti Game::ResizeButtonNearCardInfos(s32 y, s32 y2)
{
	return recti(wCardImg->getRelativePosition().LowerRightCorner.X + 10, y, 300 * window_size.Width / 1024, y2);
}
...
recti Game::ResizeCardImgWin(s32 x, s32 y, s32 x2, s32 y2) {
	float xScale = (float)window_size.Width / 1024.0;
	float yScale = (float)window_size.Height / 640.0;
	float mul = xScale;
	if (xScale > yScale)
...
void Game::LoadSkin() {
	...
	setcolor = 0xff0000ff;
	tipbackgroundcolor = 0xc0ffffff;
	selectcardcolor = 0xffffff00;
	...
	if (gameConf.skin_index >= 0)
	{
		...
		if ((size_t)gameConf.skin_index < skins.size())
		{
			...
			if (skinSystem->applySkin(skins[index].c_str()))
			{
				...
				selectcardcolor = ExtractColor(L"SelectCardColor", skinSystem, selectcardcolor);
			}
		}
	}
...
void Game::ApplySkin()
{
	...
	stTip->setBackgroundColor(tipbackgroundcolor);
	stCardListTip->setBackgroundColor(tipbackgroundcolor);
	stInfo->setOverrideColor(setcolor);
	stDataInfo->setOverrideColor(setcolor);
	stBoosterInfo->setOverrideColor(setcolor);
	env->getSkin()->setFont(guiFont);
...
void Game::LaunchBot(std::wstring deck, std::wstring name, std::wstring dialog) {
	if (deck == L"Random")
		deck = L"";
	std::wstring args = L"Name=" + name + L" Deck=" + deck + L" Dialog=" + dialog + L" Host=127.0.0.1 Port=" + std::to_wstring(gameConf.serverport) + L" Version=" + std::to_wstring(PRO_VERSION);
	STARTUPINFOW si;
	...
	myswprintf(cmd, L"Kaibot/WindBot.exe %ls", args.c_str());
	CreateProcessW(NULL, cmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, L"Kaibot", &si, &pi);
}
void Game::UpdatewCreateHostWithBot()
{
...
void Game::RefreshBotDeck(irr::gui::IGUIComboBox* cbDeck) {
	cbDeck->clear();
	cbDeck->addItem(L"Random");
	FileSystem::TraversalDir(L"./Kaibot/Decks", [cbDeck](const wchar_t* name, bool isdir) {
		if (!isdir && wcsrchr(name, '.') && !mywcsncasecmp(wcsrchr(name, '.'), L".ydk", 4)) {
			size_t len = wcslen(name);
			wchar_t deckname[256];
			wcsncpy(deckname, name, len - 4);
			deckname[len - 4] = 0;
			std::wstring decknameformat = deckname;
