...
bool ClientField::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			...
			case BUTTON_RELOAD_TEXTURES: {
				imageManager.LoadBCACustomTextures(true);
				break;
			}
			case BUTTON_SWAP_COVERS: {
				mainGame->SwapCovers(true);
				break;
			}
			case BUTTON_MSG_OK: {
				soundManager.PlaySoundEffect(SOUND_BUTTON);
			...
			case BUTTON_CARD_4: {
				...
				case MSG_SELECT_CARD: {
					...
					if (command_card->is_selected) {
						...
						mainGame->stCardPos[id - BUTTON_CARD_0]->setBackgroundColor(mainGame->selectcardcolor);
		...
		case irr::gui::EGET_SCROLL_BAR_CHANGED: {
			...
			case SCROLL_CARD_SELECT: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					irr:u32 opponentcolor = 0xff00ff00;
					// color
					if(conti_selecting)
						...
						if(selectable_cards[i + pos]->is_selected)
							...
							mainGame->stCardPos[i]->setBackgroundColor(opponentcolor);
						else
							mainGame->stCardPos[i]->setBackgroundColor(0xffffffff);
...
bool ClientField::OnCommonEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_CHECKBOX_CHANGED: {
			...
			case CHECKBOX_SHUFFLE_HAND_TEST: {
				mainGame->gameConf.shuffle_deck_test_hand = mainGame->chkShuffleHandTest->isChecked() ? 1 : 0;
				return true;
				break;
			}
			...
			case CHECKBOX_SHOW_BOOSTER_INFOS: {
				mainGame->gameConf.show_booster_infos = mainGame->chkShowBoosterTagDeckEditor->isChecked() ? 1 : 0;
				return true;
				break;
			}
			}
		...
		case irr::gui::EGET_COMBO_BOX_CHANGED: {
			switch (id) {
			case COMBOBOX_SKINS: {
				mainGame->gameConf.skin_index = mainGame->cbSkins->getSelected() - 1;
				mainGame->LoadSkin();
				mainGame->ApplySkin();
				break;
			}
			case COMBOBOX_START_HAND_COUNT_HAND_TEST: {
				mainGame->gameConf.number_card_hand_test_hand = mainGame->cbCardsCountHandTest->getSelected();
			}
			}
			break;
		}
