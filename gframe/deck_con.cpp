		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			...
			case BUTTON_SIDE_OK: {
				if(deckManager.current_deck.main.size() != pre_mainc || deckManager.current_deck.extra.size() != pre_extrac
					...
					mainGame->stACMessage->setText(dataManager.GetSysString(1410));
...
void DeckBuilder::FilterCards() {
	...
	bool isBoosterSearch = mainGame->boosters.CheckBoosterTag(str);
	...
	for(code_pointer ptr = dataManager._datas.begin(); ptr != dataManager._datas.end(); ++ptr, ++strpointer) {
		...
		if (isBoosterSearch && !mainGame->boosters.CheckCard(pstr, code, true))
			continue;
		if (mainGame->cbBoosters->getSelected() != 0)
		{
			std::wstring boostername = mainGame->cbBoosters->getItem(mainGame->cbBoosters->getSelected());
			if (!mainGame->boosters.CheckCard(boostername.c_str(), code))
				continue;
		}
...
void DeckBuilder::ClearSearch() {
	...
	mainGame->scrFilter->setVisible(false);
	mainGame->scrFilter->setPos(0);
	mainGame->cbBoosters->setSelected(0);
	ClearFilter();
	results.clear();
...
void DeckBuilder::SortList() {
	...
	case 3:
		...
		break;
	case 4:
		std::sort(left, results.end(), ClientCard::deck_sort_quantity);
		break;
	}
}
...
bool DeckBuilder::check_limit(code_pointer pointer) {
	...
	if (mainGame->chkSearchBCACardsOnly->isChecked() && !mainGame->chest.IsUnlimited() && mainGame->chest.GetCardAmount(limitcode) < limit)
