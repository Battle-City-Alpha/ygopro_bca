...
void DuelClient::ConnectTimeout(evutil_socket_t fd, short events, void* arg) {
	...
	if(!is_closing) {
		...
		mainGame->stACMessage->setText(dataManager.GetSysString(1400));
			...
			if(connect_state == 0x1) {
				...
				if(!mainGame->dInfo.isStarted && !mainGame->is_building) {
					...
					if (events & BEV_EVENT_EOF) {
						...
						mainGame->stACMessage->setText(dataManager.GetSysString(1401));
						mainGame->PopupElement(mainGame->wACMessage, 100);
					}
					else {
						//mainGame->env->addMessageBox(L"", dataManager.GetSysString(1402));
						mainGame->stACMessage->setText(dataManager.GetSysString(1402));
						mainGame->PopupElement(mainGame->wACMessage, 100);
					}
					mainGame->gMutex.unlock();
				} else {
					...
					mainGame->stACMessage->setText(dataManager.GetSysString(1502));
					mainGame->PopupElement(mainGame->wACMessage, 100);
					mainGame->btnCreateHost->setEnabled(true);
					mainGame->btnJoinHost->setEnabled(true);
					mainGame->btnJoinCancel->setEnabled(true);
...
void DuelClient::HandleSTOCPacketLan(char* data, unsigned int len) {
	...
	case STOC_ERROR_MSG: {
		...
		case ERRMSG_JOINERROR: {
			...
			if (pkt->code == 0)
				mainGame->stACMessage->setText(dataManager.GetSysString(1403));
			else if (pkt->code == 1)
				mainGame->stACMessage->setText(dataManager.GetSysString(1404));
			else if (pkt->code == 2)
				mainGame->stACMessage->setText(dataManager.GetSysString(1405));
			mainGame->PopupElement(mainGame->wACMessage, 100);
			mainGame->gMutex.unlock();
			event_base_loopbreak(client_base);
			break;
		...
		case ERRMSG_SIDEERROR: {
			...
			mainGame->stACMessage->setText(dataManager.GetSysString(1408));
			mainGame->PopupElement(mainGame->wACMessage, 100);
			mainGame->gMutex.unlock();
			break;
		}
	...
	case STOC_JOIN_GAME: {
		...
		mainGame->gMutex.lock();
		// forced mode for BCA ranked duels
		if (mainGame->gameConf.forced) {
			...
			mainGame->chkHostPrepReady[selftype]->setVisible(false);
			mainGame->chkHostPrepReady[1 - selftype]->setVisible(false);
			mainGame->btnHostPrepReady->setEnabled(false);
			mainGame->btnHostPrepNotReady->setEnabled(false);
	...
	case STOC_TYPE_CHANGE: {
		...
		if(!mainGame->dInfo.isTag) {
			...
			if(is_host && !mainGame->gameConf.forced) {
	...
	case STOC_CHAT: {
		...
		mainGame->gMutex.lock();
		mainGame->AddChatMsg(msg, player, true);
		mainGame->gMutex.unlock();
		break;
	}
	...
	case STOC_HS_PLAYER_CHANGE: {
		...
		if(state < 8) {
			...
			if(pos == selftype && !mainGame->gameConf.forced) {
				mainGame->btnHostPrepReady->setVisible(false);
		...
		if(mainGame->chkHostPrepReady[0]->isChecked() && mainGame->chkHostPrepReady[1]->isChecked()
			...
			if (mainGame->gameConf.forced)
			{
				SendPacketToServer(CTOS_HS_START);
			}
		} else {
			mainGame->btnHostPrepStart->setEnabled(false);
		}
